        ...
        <div key={childKey} className='grid grid-rows-7 grid-flow-col gap-x-1 gap-y-1 mx-auto'>
            ...
            {currMonthDateArray.map((x, dayIndex) => {
                ...
                return (
                    <div
                        key={`box-${dayIndex + 1}`}
                        className={`${isMatching ? (renderColorIntensity(frequency)) : `bg-[#a2a2a2] dark:bg-[#393939]`} ${(currDate <= dayIndex && isCurrentMonth) && "opacity-25"}  row-span-1 ${type === 'single' ? "w-[1rem] h-[1rem] rounded-[3px]" : "w-[0.58rem] h-[0.58rem]  rounded-[1px]"}`}
                        title={`${frequency} submissions on ${monthData[selectedMonth]} ${x}, ${selectedYear}`}
                    />
                );
            })}
        </div>
